...
- **Recording is state-gated**: only when lead.state ∈ `RECORD_STATES` (one-party-consent default list; CA/FL/WA etc. excluded — legal requirement, don't widen without counsel). `record-from-answer-dual` → `/twilio/recording-status` audit row.
- **Transcription**: env `TWILIO_INTELLIGENCE_SID` (create a Voice Intelligence service in Twilio console; set its webhook to `/twilio/intelligence`) → recordings ≥20s auto-transcribe → `call_transcript` audit rows.
